...
import { buildInvoiceDataModel, FORSYS_INVOICE_ENCODED, FORSYS_INVOICE_FIXTURE, INVOICE_FIXTURE, INVOICE_SERIALIZED, INVOICE_WITH_ALL_FIELDS, } from "./testdata/invoices.js";
...
describe("encode", () => {
    ...
    test("encodes all 5 document types without error", () => {
        for (const docType of Object.values(InvoiceDocumentType)) {
            const model = buildInvoiceDataModel({ documentType: docType });
    ...
    test("encodes Forsys sample invoice with matching header", () => {
        ...
        expect(decoded.supplierParty.partyName).toBe(FORSYS_INVOICE_FIXTURE.supplierParty.partyName);
        expect(decoded.customerParty.partyName).toBe(FORSYS_INVOICE_FIXTURE.customerParty.partyName);

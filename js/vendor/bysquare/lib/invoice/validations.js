...
function validateRequired(value, path) {
...
export function validateDataModel(model) {
    ...
    validateRequired(model.supplierParty.postalAddress.streetName, "supplierParty.postalAddress.streetName");
    validateRequired(model.supplierParty.postalAddress.cityName, "supplierParty.postalAddress.cityName");
    validateRequired(model.supplierParty.postalAddress.postalZone, "supplierParty.postalAddress.postalZone");
    validateRequired(model.supplierParty.postalAddress.country, "supplierParty.postalAddress.country");
    if (model.supplierParty.postalAddress.country
    ...
    if (hasLineCount && model.numberOfInvoiceLines <= 0) {
        throw new ValidationError(ErrorMessages.NumberOfInvoiceLinesPositive, "numberOfInvoiceLines");
    }
    ...
    if (model.singleInvoiceLine) {
        ...
        if (hasFrom !== hasTo) {
            throw new ValidationError(ErrorMessages.PeriodDateConsistency, "singleInvoiceLine.periodFromDate");
        }
        if (hasFrom && hasTo) {
    ...
    if (!model.taxCategorySummaries || model.taxCategorySummaries.length === 0) {
        throw new ValidationError(ErrorMessages.TaxCategorySummariesEmpty, "taxCategorySummaries");
    }
    for (const [index, summary] of model.taxCategorySummaries.entries()) {
        const path = `taxCategorySummaries[${index}]`;
        if (summary.classifiedTaxCategory === undefined
            || summary.classifiedTaxCategory < 0
            || summary.classifiedTaxCategory > 1) {
            throw new ValidationError(ErrorMessages.ClassifiedTaxCategory, `${path}.classifiedTaxCategory`);
        }
    }
}

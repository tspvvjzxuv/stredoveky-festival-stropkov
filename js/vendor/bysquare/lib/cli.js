...
import { encode as invoiceEncode, } from "./invoice/encode.js";
import { decode as payDecode } from "./pay/decode.js";
import { encode as payEncode, } from "./pay/encode.js";
...
async function readQrInput(args) {
    if (args.length === 0) {
...
async function cmdPayEncode(args) {
...
function payEncodeAndPrint(jsonStr, opts) {
    try {
...
async function cmdPayDecode(args) {
    try {
...
async function cmdInvoiceEncode(args) {
    const parsed = parseArgs({
        args: args,
        allowPositionals: true,
        options: {
...
async function cmdInvoiceDecode(args) {
    try {
        const qr = await readQrInput(args);
...
async function cmdDecodeAuto(args) {
    try {
        const qr = await readQrInput(args);

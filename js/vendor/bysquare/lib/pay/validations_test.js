...
import { buildDataModel, buildPaymentOrder, CURRENCY_TEST_CASES, IBAN_TEST_CASES, MINIMAL_PAYMENT, TEST_BICS, TEST_IBANS, VALID_PAYMENT_ORDER, } from "./testdata/index.js";
import { CurrencyCode, PaymentOptions, } from "./types.js";
import { validateBankAccount, validateDataModel, validateSimplePayment, } from "./validations.js";
...
describe("validateSimplePayment", () => {
    ...
    test("rejects payment without beneficiary", () => {
        const payment = buildPaymentOrder({ beneficiary: undefined });
        expect(() => validateSimplePayment(payment, "test")).toThrow(ValidationError);
    });
...
describe("validateDataModel", () => {
    ...
    test("rejects data model with invalid payment", () => {
        const dataModel = buildDataModel({
            payments: [buildPaymentOrder({ currencyCode: "INVALID" })],
        });
    ...
    test("provides correct error path for nested validation errors", () => {
        ...
        try {
            ...
            expect(error.path).toBe("payments[0].bankAccounts[0].iban");
        }
    });
});
describe("ValidationError", () => {
    test("contains correct message and path for currency error", () => {
        const dataModel = buildDataModel({
            payments: [buildPaymentOrder({ currencyCode: "NOTACURRENCY" })],
        });
        try {
            validateDataModel(dataModel);
            expect.unreachable("Should have thrown");
            ...
            expect(error.path).toBe("payments[0].currencyCode");
        }
    });
    test("contains correct message and path for IBAN error", () => {
        ...
        try {
            ...
            expect(error).toBeInstanceOf(ValidationError);
            expect(error.message).toContain("Invalid IBAN");
            expect(error.path).toBe("test.bankAccount.iban");
        }
    });
});
/**
...
describe("permissive validation behavior", () => {
    test("accepts lowercase currency codes (validator is case-insensitive)", () => {
        const payment = buildPaymentOrder({ currencyCode: "eur" });
        expect(() => validateSimplePayment(payment, "test")).not.toThrow();
    });
    test("accepts XXX currency code (valid ISO 4217 'no currency')", () => {
        const payment = buildPaymentOrder({ currencyCode: "XXX" });
        expect(() => validateSimplePayment(payment, "test")).not.toThrow();
    });
